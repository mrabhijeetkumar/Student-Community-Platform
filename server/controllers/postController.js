...
export const getUserPosts = async (req, res) => {
    try {
        const targetUser = await User.findById(req.params.userId).select("isPrivate followers");
        ...
        const isFollower = targetUser.followers.some((followerId) => followerId.toString() === req.user._id.toString());
        const isAdmin = req.user.role === "admin";

        if (targetUser.isPrivate && !isOwner && !isFollower && !isAdmin) {
...
export const searchPosts = async (req, res) => {
    try {
        ...
        const filter = {};
        if (q) {
            const safeRegex = buildSafeContainsRegex(q);
            filter.$or = [
                { content: safeRegex },
                { tags: safeRegex }
            ];
        ...
        if (tag) {
            filter.tags = buildSafeContainsRegex(tag);
        }
