...
import { buildSafeContainsRegex } from "../utils/regexUtils.js";

// NOTE: "email" is intentionally excluded from the public select list.
// Email addresses are private account data and must never be exposed to
// other users through profile/search/follow endpoints — only the owner
// should see their own email (handled separately in getCurrentUser).
const publicUserSelect = "username name profilePhoto coverPhoto headline bio college skills socialLinks followers following followRequestsReceived followRequestsSent isPrivate role roles authProvider isEmailVerified createdAt updatedAt";
const ownerUserSelect = `${publicUserSelect} email`;
...
export const getCurrentUser = async (req, res) => {
    const user = await User.findById(req.user._id).select(ownerUserSelect);
...
export const updateCurrentUser = async (req, res) => {
    try {
        ...
        const updatedUser = await User.findById(user._id).select(ownerUserSelect);
...
export const getUserDirectory = async (req, res) => {
    try {
        ...
        if (query) {
            const safeRegex = buildSafeContainsRegex(query);
            filter.$or = [
                { name: safeRegex },
                { username: safeRegex },
                { college: safeRegex },
...
export const removeFollower = async (req, res) => {
    ...
    const refreshedUser = await User.findOne({ username: currentUser.username }).select(ownerUserSelect);
    res.json({
        ...formatProfileResponse(refreshedUser, req.user._id),
...
export const getUserFollowers = async (req, res) => {
    try {
        ...
        if (user.isPrivate && !isOwner && !isAdmin) {
            const viewerIsFollower = user.followers.some((follower) => (follower._id || follower).toString() === req.user._id.toString());
            if (!viewerIsFollower) {
...
export const getUserFollowing = async (req, res) => {
    try {
        ...
        if (user.isPrivate && !isOwner && !isAdmin) {
            const viewerIsFollower = user.followers.some((followerId) => followerId.toString() === req.user._id.toString());
            if (!viewerIsFollower) {
                return res.status(403).json({ message: "This account is private" });
            }
        }
