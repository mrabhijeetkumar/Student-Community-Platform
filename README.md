...
REACT_APP_EMAIL_VERIFICATION_WEBHOOK=<your-email-sender-endpoint-required>
```
...
OTP email delivery is mandatory for signup verification. Configure `REACT_APP_EMAIL_VERIFICATION_WEBHOOK` to send OTP emails to user Gmail inbox.
Direct account creation API is disabled; account is created only after successful OTP verification.
